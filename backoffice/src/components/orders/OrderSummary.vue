...
const emit = defineEmits<{
  'update-shipping': [amount: number]
  'update-discount': [amount: number]
}>()

// État local
const localShippingAmount = ref(props.shippingAmount || 0)
...
const totalHtAfterDiscount = computed(() => {
  return Math.max(0, subtotalHt.value + localShippingAmount.value - localDiscountAmount.value)
})
...
const updateShipping = (event: Event) => {
  ...
  localShippingAmount.value = value
...
// Watchers
watch(() => props.shippingAmount, (value) => {
  if (value !== undefined) {
    localShippingAmount.value = value
  }
})
