...
/**
 * Intégration Gmail via l'API REST v1 (OAuth2 refresh_token).
 */
@Service
...
public class GoogleMailGmailApiService implements GoogleMailService {
    ...
    public List<GmailMessageSimple> getRecentEmails(GoogleOAuthCredentials credentials,
        ...
        if (!fetched.isEmpty() && filtered.isEmpty()) {
            log.warn("Aucun message après filtre date depuis {} — utilisation des {} dernier(s) reçu(s)",
                    since, fetched.size());
            return fetched;
        }
        return filtered;
    ...
    public byte[] getAttachmentData(GoogleOAuthCredentials credentials, String messageId, String attachmentId) {
        ...
        try {
            ...
            log.warn("Pièce jointe Gmail introuvable: messageId={} attachmentId={}", messageId, attachmentId, e);
            return new byte[0];
        }
    }
    ...
    public String createLabel(GoogleOAuthCredentials credentials, String labelName) {
        ...
        try {
            ...
            throw new IllegalStateException("Réponse création libellé invalide", e);
        }
    }
    ...
    private GmailMessageSimple fetchOneMessage(String token, String id) {
        ...
        try {
            ...
            log.warn("Lecture message Gmail id={} ignorée", id, e);
            return null;
        }
    }

    /**
     * Derniers messages (ordre décroissant côté Gmail), sans {@code q} pour éviter des listes vides.
     */
    ...
    private List<GmailLabel> parseLabels(String body) throws Exception {
        ...
        log.debug("Libellés Gmail personnalisés : {}", out.size());
        return out;
    }
